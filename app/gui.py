...
class AudioApp:
    ...
    def setup(self):
        ...
        self.root.configure(bg="skyblue") # hexadecimal value for royal blue background
        
        self.bold_font = font.Font(family="Courier New", size=18, weight="bold")
        # Title label design
        highlightFont = font.Font(family="Courier New", size=60, weight="bold")
        # Text and title 
       
        # Top frame
        top_frame = tk.Frame(self.root, bg="#000080", height = 50)
        top_frame.pack(fill="x")
        #======================================
        title = tk.Label(self.root, text="EQLZR", font=highlightFont,fg="#F4A460", bg="#000080")
    ...
    def volume_label(self):
        # Input to take in
        volume_label = tk.Label(self.root, text="Enter Volume (1-100):", bg="#4169E1", foreground="#F4A460", font=self.bold_font) # label for input
        volume_label.pack()
        
        vcmd = (self.root.register(self.validate_volume_input), '%P')
        self.volume_entry = tk.Entry(self.root, justify="center", font=("Courier New", 24)) # input label
    ...
    # Determine is on or off
            if self.is_on:
                my_label.config(text = "EQLZR is Off", fg = "grey")
                on_button.config(text="OFF", font=self.bold_font) 
                self.is_on = False
            else:
                my_label.config(text = "EQLZR is On!", fg = "green")
